...
import com.hishixi.tiku.custom.recycler.adapter.BasicAdapter;
...
import com.hishixi.tiku.custom.recycler.listener.OnItemClickListener;
import com.hishixi.tiku.custom.recycler.listener.OnItemLongClickListener;
...
public class BasicRecyclerView extends RecyclerView {
    ...
    private int divider_marginLeft, divider_marginRight, divider_marginTop,
            divider_marginBottom;
    ...
    private void initStyleable(Context context, AttributeSet attrs) {
        ...
        has_row_divider = array.getBoolean(R.styleable
                .BasicRecyclerView_has_row_divider, false);
        has_rank_divider = array.getBoolean(R.styleable
                .BasicRecyclerView_has_rank_divider, false);

        isInner = array.getBoolean(R.styleable.BasicRecyclerView_is_inner, false);
        divider_width = (int) array.getDimension(R.styleable
                .BasicRecyclerView_divider_width, 1f);
        divider_height = (int) array.getDimension(R.styleable
                .BasicRecyclerView_divider_height, 1f);
        divider_marginLeft = (int) array.getDimension(R.styleable
                .BasicRecyclerView_divider_marginLeft, 0f);
        divider_marginRight = (int) array.getDimension(R.styleable
                .BasicRecyclerView_divider_marginRight, 0f);
        divider_marginTop = (int) array.getDimension(R.styleable
                .BasicRecyclerView_divider_marginTop, 0f);
        divider_marginBottom = (int) array.getDimension(R.styleable
                .BasicRecyclerView_divider_marginBottom, 0f);
        divider_color = array.getColor(R.styleable.BasicRecyclerView_divider_color,
                Color.parseColor("#00000000"));
        isProcess = array.getBoolean(R.styleable.BasicRecyclerView_is_process, false);
        mProcessBackground = array.getInteger(R.styleable
                .BasicRecyclerView_process_background, R.drawable
                .recyclerview_background);
        isStartAnimator = array.getBoolean(R.styleable
                .BasicRecyclerView_is_start_animator, false);
        mDuration = array.getInt(R.styleable.BasicRecyclerView_duration_animator, 300);
        isFirstOnly = array.getBoolean(R.styleable.BasicRecyclerView_is_first_only,
                false);
        array.recycle();
    }
    ...
    private void initView(Context context) {
        ...
        switch (layout_mode) {
            case BasicRecyclerView.HORIZONTAL:
                if (!isInner)
                    manager = new LinearLayoutManager(context, LinearLayoutManager
                            .HORIZONTAL, false);
                else
                    manager = new InnerLinearLayoutManager(context, LinearLayoutManager
                            .HORIZONTAL, false);
                break;
...
//                    ((GridLayoutManager) manager).setSmoothScrollbarEnabled(true);
        ...
        //添加间隔线
        if (has_row_divider) {
            HorizontalDividerItemDecoration decoration = new
        ...
        if (has_rank_divider) {
            VerticalDividerItemDecoration decoration = new
     ...
     * 设置长点击监听
     *
     * @param onItemLongClickListener 长点击监听器
     */
